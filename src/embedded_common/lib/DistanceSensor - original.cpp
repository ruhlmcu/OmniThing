...
//private
    DistanceSensor* DistanceSensor::createFromJson(const char* json)
    {
        ...
        if(json_scanf(json, len, "{pin: %hu, distance: %f}", &pin, &distance)!=3)
            return nullptr;

        LOG << F("Distance = ") << distance << Logger::endl;
        return new DistanceSensor(pin, distance);
    }

//protected
    unsigned int DistanceSensor::readPin()
    {
        return analogRead(getPin());
    }

//public
    DistanceSensor::DistanceSensor(unsigned short pin, float minVolt, float maxVolt):
    ...
    InputUInt* DistanceSensor::createUIntFromJson(const char* json)
    {
        return createFromJson(json);
    }

    InputFloat* DistanceSensor::createFloatFromJson(const char* json)
    {
        return createFromJson(json);
    }

    const char* DistanceSensor::Type = "DistanceSensor";
    ObjectConfig<InputUInt> DistanceSensor::InputUIntConf(Type, createUIntFromJson);
    ObjectConfig<InputFloat> DistanceSensor::InputFloatConf(Type, createFloatFromJson);
}
